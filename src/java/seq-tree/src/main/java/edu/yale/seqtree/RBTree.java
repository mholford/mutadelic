...
public class RBTree {
	...
	public void insertNode(RBNode x) {
		...
		while (!(x.equals(getRoot())) && (x.getParent().getColor() == RBColor.RED)) {
//			System.out.println(String.format("%s != %s", ((IntervalNode) x)
//					.getInterval(), ((IntervalNode) root).getInterval()));
			RBNode parent = x.getParent();
			RBNode grandparent = parent.getParent();
			if (x.getParent().equals(grandparent.getLeft())) {
				RBNode y = grandparent.getRight();
				if ((y != null) && (y.getColor() == RBColor.RED)) {
					...
					x.getParent().setColor(RBColor.BLACK);
					grandparent.setColor(RBColor.RED);
					shouldPropagate = rightRotate(grandparent);
					propagateStart = x.getParent();
				}
			} else {
				// Same as then clause with "right" and "left" exchanged
				RBNode y = grandparent.getLeft();
				if ((y != null) && (y.getColor() == RBColor.RED)) {
					// Case 1
					if (DEBUGGING && REALLY_VERBOSE) {
					...
					x.getParent().setColor(RBColor.BLACK);
					grandparent.setColor(RBColor.RED);
					shouldPropagate = leftRotate(grandparent);
					propagateStart = x.getParent();
				}
			}
		}

		while (shouldPropagate && !(propagateStart.equals(getRoot()))) {
//			System.out.println(String.format("%s != %s",
//					((IntervalNode) propagateStart).getInterval(),
//					((IntervalNode) root).getInterval()));
			if (DEBUGGING && REALLY_VERBOSE) {
	...
	/**
	 * FIXME: this does not work properly yet for augmented red-black trees
	 * since it doesn't update nodes. Need to figure out exactly from which
	 * points we need to propagate updates upwards.
	 */
	...
	protected boolean leftRotate(RBNode x) {
		// Set y.
		RBNode y = x.getRight();
	...
	/** Returns true if updates must continue propagating up the tree */
	protected boolean rightRotate(RBNode y) {
		// Set x.
		RBNode x = y.getLeft();
	...
	/**
	 * Restores red-black property to tree after splicing out node during
	 * deletion. Note that x may be null, which is why xParent must be
	 * specified.
	 */
		...
		while ((x != root) && ((x == null) || (x.getColor() == RBColor.BLACK))) {
			if (x == xParent.getLeft()) {
				...
				if (((w.getLeft() == null) || (w.getLeft().getColor() == RBColor.BLACK))
					...
					if ((w.getRight() == null)
							|| (w.getRight().getColor() == RBColor.BLACK)) {
						// Case 3
				...
				if (((w.getRight() == null) || (w.getRight().getColor() == RBColor.BLACK))
					...
					if ((w.getLeft() == null)
							|| (w.getLeft().getColor() == RBColor.BLACK)) {
						// Case 3
						w.getRight().setColor(RBColor.BLACK);
						w.setColor(RBColor.RED);
						leftRotate(w);
