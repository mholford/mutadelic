...
public class getAllPhyloPs {
   public static final String FILE_SUFFIX = ".phyloP46way.wigFix.gz";
   public static final String OUTPUT_FILE = "phyloP-output";
   public static final String ADDRESS = "hgdownload.cse.ucsc.edu/goldenpath/hg19/phyloP46way/vertebrate";

   public static void main(String[] args) {
      new getAllPhyloPs().exec();
   }
   ...
   private File dlFile(String fname) throws IOException, InterruptedException {
      ...
      File outFile = new File("phylop-" + fname + "-input.gz");
      List<String> inv = new ArrayList<String>();
      inv.add("curl");
      inv.add("-s");
      inv.add("-o");
      inv.add("phylop-" + fname + "-input.gz");
   ...
   private void parseFile(String chr, File f) throws IOException {
      ...
      while ((s = br.readLine()) != null) {
         ...
         if (s.startsWith("fixedStep")) {
            ...
            ofWriter.write(String.format("%s\t%d\t%10.4f\n", chr, num, val));
            ofWriter.flush();
            ++num;
         }
      }
